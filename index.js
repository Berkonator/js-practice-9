...
function findIndexByProperty(arr, name) {
  const found = arr.findIndex(indexOk => indexOk === name)
  if(found === -1){
    return 1
  }
...
function sumAndPrint(arr) {
  if(Array.isArray(arr)){
    ...
    const a = 0;
    const sum = arr.reduce((x, y) => x + y + a)
    arr.forEach(element => {
      console.log(element)
    });
    return sum;
...
function flattenSquareAndFilter(arr, threshold) {
  if(Array.isArray(arr)){
    const m1 = arr.flat();
    const m2 = m1.map(x => Math.pow(x, 2));
    const m3 = m2.filter(y => y > threshold);
    return m3;
  };
  return 'Вхідний параметр має бути масивом';
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  if(Array.isArray(arr)){
    let x = arr.length - n;
    const m1 = arr.slice(x);
    const m2 = m1.reduceRight((a, b) => a + b);
    const m3 = m1.forEach(element => console.log(element))
    return m2;
  };
  return 'Вхідний параметр має бути масивом';
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function calculateTotalPrice(arr) {
  if(Array.isArray(arr)){
    return arr.reduce((prevPrice, currentOject) => {
     return currentOject.price + prevPrice;
    }, 0)
  };
  return 'Вхідний параметр має бути масивом';
